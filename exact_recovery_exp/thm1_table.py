...
K0 = 2 #max freq of fourier features
fmax = 2*K0+1
freq = torch.fft.fftfreq(fmax,d=1/fmax)
freq_x,freq_y = torch.meshgrid(freq,freq)
mask0 = (torch.max(torch.abs(freq_x),torch.abs(freq_y)) <= K0) & (freq_x >= 0) & ~((freq_y<=0) & (freq_x == 0)) #selects frequencies in a half space, skip (0,0) frequency
...
def train_student(rlayer, avec, width_s, mask, y, param, std_wd=True):
  ...
  for k in range(outer_iters): #outer loop for updating lagrange multiplier/increasing mu
    ...
    for iter in range(inner_iters):
      hiddenout = hidden(coords).T.view([width_s,res[0],res[1]]) #hidden layer output imgs [width,nx,ny]
      Ahidden = torch.fft.fft2(hiddenout,norm="forward")[:,mask] #apply A operator to every hidden layer img [width,len(y)]
      Axreal = avec@torch.real(Ahidden) #separate real and imaginary parts
      Aximag = avec@torch.imag(Ahidden)

      if std_wd:
        wdloss = 0.5*((rlayer[0].weight**2).sum() + (avec**2).sum())
      else:
        wdloss = 0.5*((torch.real(Ahidden)**2 + torch.imag(Ahidden)**2).sum() + (avec**2).sum())
      ...
      with torch.no_grad():
        ...
        if std_wd:
          ...
          virtual_loss = (torch.norm(Ahidden,dim=1)*torch.abs(avec)).sum()
...
for W in (width_values):
    for K in (k_values):
        ...
        param = {"mu_init": 10, "mu_inc_factor": 1.5, "outer_iters": 60, "inner_iters": 5000, "lr": 0.001}
